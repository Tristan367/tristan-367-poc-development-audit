...
test("CharacterNamePut returns 204 on success", async () => {
...
test("CharacterNamePut returns 404 when movie not found", async () => {
...
test("CharacterNamePut returns 404 when character not found", async () => {
  const movie = makeMovie(movieId, []);
  const MovieModel = { find: jest.fn().mockResolvedValue([movie]) };
...
test("CharacterNamePut returns 406 when name has less than 3 characters", async () => {
